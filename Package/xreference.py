...
def create_reference_transfer_config_file(local_dir='./data', selected_file_list=['GCF_000146045.2_R64_genomic.fna.gz'], reference_dataset_id = 'Scerevisiae'):
    ...
    try:
        ...
        with open(reference_transfer_config_file, mode='w', encoding='utf8') as file_id:
            ...
            file_id.write('{0}\n'.format('# The reference files must be located in the cluster directory {0}/reference_dataset_id'.format(xlib.get_cluster_reference_dir())))
...
def upload_reference_dataset(cluster_name, log, function=None):
    ...
    log.write('The reference transfer config file is been validating ...\n')
    if validate_reference_transfer_config_file(strict=True):
...
def validate_reference_transfer_config_file(strict):
    '''
    Validate the reference transfer config file.
    '''
...
def get_reference_file_name_list(cluster_name, reference_dataset_id, passed_connection=False, ssh_client=None):
    ...
    # verify the reference directory is created
    if OK:
        command = '[ -d {0} ] && echo RC=0 || echo RC=1'.format(cluster_reference_dir)
        (OK, stdout, stderr) = xssh.execute_cluster_command(ssh_client, command)
        if stdout[len(stdout) - 1] != 'RC=0':
