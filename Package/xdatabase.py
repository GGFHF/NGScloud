...
def create_database_transfer_config_file(local_dir='./data', selected_file_list=['RefSeq_Plan_Protein.faa', 'RefSeq_Plan_Protein.pal'], database_dataset_id = 'RefSeq_Plan_Protein'):
    ...
    try:
        ...
        with open(get_database_transfer_config_file(), mode='w', encoding='utf8') as file_id:
...
def upload_database_dataset(cluster_name, log, function=None):
    ...
    log.write('The database transfer config file is been validating ...\n')
    if validate_database_transfer_config_file(strict=True):
    ...
    if OK:
        ...
        for section in sections_list:
            ...
            if re.match('^file-[0-9]+$', section):
                ...
                # upload the database file in the cluster
                log.write('{0}\n'.format(xlib.get_separator()))
...
def validate_database_transfer_config_file(strict):
    '''
    Validate the database transfer config file.
    '''
...
def get_database_dataset_dict(cluster_name, passed_connection=False, ssh_client=None):
    ...
    # verify the database directory is created
    if OK:
